...
/* AS608指纹相关 */
u8 as608_proc_flag = 0;                     // WAK引脚状态
u8 as608_proc_flag_old = 0;                 // 上一次WAK状态
u16 as608_store_index = 0;                  // 指纹存储索引
u16 as608_match_page = 0;                   // 匹配到的指纹ID
u16 as608_match_score = 0;                  // 匹配分数
u8 as608_busy = 0;                          // 指纹处理忙标志
...
task_t scheduler_task[] = {
    ...
    {As608_Proc,   200, 0},  // 指纹处理，200ms（检测WAK状态变化）
};
        ...
        case 16:  // 确认键
            switch(mode)
            {
                ...
                case 5:
                    ...
                    {
                        mode = 6;           // 进入录入指纹状态
                        Audio_Play(19);     // "管理员密码验证成功，请将手指平放在传感器上"
                        Key_Clear();
                    }
                    else
                    {
...
void As608_Proc(void)
{
    ...
    if(mode == 6)
    {
        ...
        // 第一次按手指
        result = PS_GetImage();
        if(result != 0) { as608_busy = 0; return; }
        result = Ps_GenChar(1);
        if(result != 0) { as608_busy = 0; return; }
        delay_ms(1000);
        ...
        // 存储模板
        result = PS_StoreChar(2, as608_store_index);
        if(result != 0) { as608_busy = 0; return; }
        
        as608_store_index++;
        Audio_Play(21);  // "指纹录入成功"
        mode = 0;
        as608_busy = 0;
        return;
    }
    ...
    if(mode == 0 && !as608_busy)
    {
        ...
        // 搜索指纹库
        result = PS_Search(2, 0, 300, &as608_match_page, &as608_match_score);
        if(result == 0 && as608_match_score > 50)  // 匹配成功且分数>50
        {
            Audio_Play(16);      // "指纹验证成功，门已开启，欢迎回家"
            lock_flag = 0;       // 开锁
            show_flag = 1;       // 显示门已开启
        }
        else
        {
            Audio_Play(17);      // "指纹验证失败，请重试"
        }
